<template>
    <main>
        ...
        <van-tabs v-model="activeName" @click="onClick" style="padding-top: 50px;" animated>
            ...
            switch(this.activeName){
                    ...
                    params:{
                        ...
                        for(let i=0;i<res.data.length;i++){
                            this.$axios.get('/recipeList',{
                                params:{
                                    status:0,  //只显示已发布的
                                    _id:res.data[i].favorite
                                }
                            }).then(resp=>{
                                this.recipedata = this.recipedata.concat(resp.data)
                            })
                        }
                    })
                break;
                case 'news':
                    ...
                    params:{
                        ...
                        console.log(res.data,'收藏wenzhang')
                        for(let i=0;i<res.data.length;i++){
                            this.$axios.get('/newsCollect',{
                                ...
                                this.newsdata = this.newsdata.concat(resp.data)
                            })
                        }
                    })
                break;
